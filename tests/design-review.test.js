...
test('Brief Review 对待确认内容标记 Needs Evidence', () => {
  ...
  reasoning.visualDNA.photography = '摄影方向待确认';
  ...
  assert.equal(review.checks.find((item) => item.section === 'Visual DNA').status, 'Needs Evidence');
  assert.equal(review.checks.find((item) => item.section === 'Emotional Direction').status, 'Needs Evidence');
  assert.ok(review.openQuestions.some((item) => item.startsWith('Visual DNA：')));
});

test('v3.1 流水线不再写入成长历史或使用 reviewScores', async () => {
