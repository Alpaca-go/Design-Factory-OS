...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (['--output', '-o', '--config', '-c', '--name'].includes(arg)) {
      const value = args[++i];
      if (!value || value.startsWith('-')) throw new Error(`${arg} 缺少参数值`);
      const key = ({ '--output': 'output', '-o': 'output', '--config': 'config', '-c': 'config', '--name': 'name' })[arg];
      options[key] = value;
